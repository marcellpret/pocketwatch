<template>
  ...
  <div v-else>
    <SummaryCard
      ...
      @action="openAdd"
    >
      <template v-if="budgetedRows.length > 0">
      ...
      <div v-else-if="expenseCategories.length === 0" class="py-1 text-sm text-white/80">
        No expense categories yet.
        <NuxtLink to="/categories" class="font-semibold text-white underline-offset-2 hover:underline">Add some →</NuxtLink>
      </div>

      <div v-else class="py-1 text-sm text-white/80">
        No budgets yet. Set how much you want to spend on a category each month.
      </div>
    </SummaryCard>

    <template v-if="budgetedRows.length > 0">
      <div class="mb-4 text-xs font-medium uppercase tracking-wide text-muted-foreground">
